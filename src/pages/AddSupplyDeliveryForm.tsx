...
//   build: (
//     refs: RefMap,
//     row: RowDeliveryInput,
//     ctx: RowDeliveryBatchContext,
//   ) => SuperBatchSubRequest;
// }
// const DELIVERY_CHAIN: ChainStep[] = [
//   productStep,
//   supplyDeliveryStep,
//   inwardItemStep,
// ];
// DELIVERY_CHAIN.forEach((step) => {
//   requests.push(
//     step.build(refs, row, ctx),
//   );
// });
...
//     try {
//       // Convert rows to RowDeliveryInput format
...
//       // Process each chunk (but rows within chunk run in parallel on server)
...
//             throw new Error(`Chain ${result.chainId} failed: ${result.errors.join("; ")}`);
//           }
//         }
//       }
...
// ============================================================================
// CHANGE 1: UPDATE IMPORTS (lines ~27-32)
// ============================================================================
// OLD:
// import {
//   buildRowDeliveryBatch,
...
// ─── Types ─────────────────────────────────────────────────────────────────
// DELETE THIS LINE:
// export const SUPER_BATCH_CHAIN_SIZE = 15;  // ← REMOVE THIS
...
export default function AddSupplyDeliveryForm({
  ...
  // =========================================================================
  // CHANGE 2: REPLACE handleSave() FUNCTION
  // =========================================================================
  // OLD: handleSave() was calling buildRowDeliveryBatch() per row
  // NEW: handleSave() calls buildChainBatch() per chunk
  ...
  async function handleSave() {
    ...
    try {
      // Step 1: Convert RowItem[] to RowDeliveryInput[] (same as before)
      const deliveryInputs: RowDeliveryInput[] = rows.map((row) => ({
        productKnowledgeSlug: row.product_knowledge_slug,
        productKnowledgeName: row.product_knowledge_name,
        chargeItemCategorySlug: row.charge_item_category_slug,
      ...
      // Step 2: Split rows into chunks (NEW!)
      // Each chunk will be processed in parallel on the server
      const chunks = chunkRows(deliveryInputs);

      // Step 3: Create shared context (same as before)
      const ctx: RowDeliveryBatchContext = {
        facilityId,
        destination,
        deliveryOrderId,
      ...
      // But rows WITHIN each chunk run in parallel on the server!
      for (const chunk of chunks) {
        // Build ONE payload for the entire chunk (NEW!)
        // Instead of: await Promise.all(chunk.map(...))
        const payload = buildChainBatch(chunk, ctx);

        // Send ONE HTTP request for up to 8 rows
        const results = await runSuperBatch(payload);

        // Parse results by chain (NEW!)
        const chainResults = extractChainResults(results);
