...
export function buildRowDeliveryBatch(
  ...
  if (!reuseExistingProduct) {
    // 1) charge item definition -> $.slug
    requests.push({
      reference_id: "create_charge_item",
      url: `/api/v1/facility/${ctx.facilityId}/charge_item_definition/`,
      method: HttpMethod.POST,
      body: {
        slug_value: crypto.randomUUID(),
        category: input.chargeItemCategorySlug,
        title: `${input.productKnowledgeName} - ${input.batchNumber}`,
        status: "active",
        can_edit_charge_item: false,
        price_components: [
          { monetary_component_type: "base", amount: input.purchasePrice ?? "0" },
        ],
        discount_configuration: null,
      },
    });

    // 2) product -> $.id  (charge_item_definition <- step 1 $.slug)
    requests.push({
